...
from model import MentionDetection
...
train, dev, test = SimpleQuestionDataset.splits(TEXT, ED, args.data_dir)
TEXT.build_vocab(train, dev, test)
ED.build_vocab(train, dev, test)

match_embedding = 0
if os.path.isfile(args.vector_cache):
    stoi, vectors, dim = torch.load(args.vector_cache)  # stoi?
        ...
        if iterations % args.dev_every == 0:
            ...
            if args.dataset == 'EntityDetection':
                if F > best_dev_F:
                    ...
                    best_dev_R = R
                    iters_not_improved = 0
                    snapshot_path = os.path.join(save_path, args.specify_prefix + '_best_model_cpu.pt')
